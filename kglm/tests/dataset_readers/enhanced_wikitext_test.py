...
class TestEnhancedWikitextEntityNLMReader:
    ...
    def test_read_from_file(self, lazy):
        ...
        assert first_instance_tokens[:5] == [
            '@@START@@', 'State', 'Route', '127', '(']
        assert first_instance_tokens[-5:] == ['Elmer',
                                              'Huntley', 'Bridge', '.', '@@END@@']
        second_instance_entity_types = instances[1]["entity_types"].array
        np.testing.assert_allclose(
            second_instance_entity_types[:5], [0, 0, 1, 1, 1])
        np.testing.assert_allclose(
            second_instance_entity_types[-5:], [0, 0, 0, 0, 0])
        np.testing.assert_allclose(
            instances[1]["entity_ids"].array[:5], [0, 0, 1, 1, 1])
        np.testing.assert_allclose(
            instances[1]["entity_ids"].array[-5:], [0, 0, 0, 0, 0])
...
class TestEnhancedWikitextKglmReader:
    ...
    def test_read_from_file(self, lazy, mode):
        ...
        first_instance_source_tokens = [
            x.text for x in instances[0]['source'].tokens]
        assert first_instance_source_tokens[:5] == [
            '@@START@@', 'State', 'Route', '127', '(']
        assert first_instance_source_tokens[-5:] == ['the',
                                                     'Elmer', 'Huntley', 'Bridge', '.']
        ...
        if mode == "discriminative":
            ...
            # Test target tokens
            first_instance_target_tokens = [
                x.text for x in instances[0]['target'].tokens]
            assert first_instance_target_tokens[:5] == [
                'State', 'Route', '127', '(', 'SR']
            assert first_instance_target_tokens[-5:] == [
                'Elmer', 'Huntley', 'Bridge', '.', '@@END@@']
        ...
        # Test entity id
        first_instance_entity_ids = [
            x.text for x in instances[0]['entity_ids'].tokens]
        # Non-mentions correspond to padding tokens
        assert first_instance_entity_ids[7+offset:9 +
                                         offset] == ['@@PADDING@@', '@@PADDING@@']
        assert first_instance_entity_ids[7+offset:9 +
                                         offset] == ['@@PADDING@@', '@@PADDING@@']
        # Mentions are the WikiData id
        assert first_instance_entity_ids[16 +
                                         offset:18+offset] == ['Q831285', 'Q831285']
        ...
        first_instance_parent_ids = [[x.text for x in y.tokens]
                                     for y in instances[0]['parent_ids']]
        first_instance_relations = [[x.text for x in y.tokens]
                                    for y in instances[0]['relations']]
        ...
        first_instance_shortlist = [
            x.text for x in instances[0]['shortlist'].tokens]
        expected_entities = {
        ...
        assert first_instance_shortlist_inds[16 +
                                             offset] == first_instance_shortlist.index('Q831285')
