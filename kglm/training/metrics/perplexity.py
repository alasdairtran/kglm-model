...
class Perplexity(Metric):
    ...
    def __call__(self,
        ...
        log_p = torch.gather(
            log_p, dim=2, index=labels.unsqueeze(2)).squeeze(2)
        if mask is not None:
            self._sum_log_p += (mask.float() * log_p).sum()
            self._total_count += mask.float().sum()
        else:
            self._sum_log_p += log_p.sum()
            self._total_count += torch.numel(
                labels)  # pylint: disable=no-member
...
class UnknownPenalizedPerplexity(Metric):
    ...
    def __init__(self,
        ...
        if unk_vocab_size > 0:
            self._unk_penalty = math.log(
                unk_vocab_size)  # pylint: disable=no-member
        else:
            self._unk_penalty = 0.0

        # Identify the index of the <unk> token.
        self._unk_idx = vocabulary.get_token_index(
            oov_token, namespace=namespace)
