...
class SplitCrossEntropyLoss(nn.Module):
    ...
    def logprob(self, weight, bias, hiddens, splits=None, softmaxed_head_res=None, verbose=False):
        ...
        if softmaxed_head_res is None:
            ...
            softmaxed_head_res = torch.nn.functional.log_softmax(
                head_res, dim=-1)
        ...
        for idx in splits:
            ...
            # If the target is in one of the splits, the probability is the p(tombstone) * p(word within tombstone)
                ...
                tail_entropy = torch.nn.functional.log_softmax(
                    tail_res, dim=-1)
    ...
    def forward(self, weight, bias, hiddens, targets, verbose=False):
        ...
        combo = torch.cat([split_hiddens[i]
                           for i in range(self.nsplits) if len(split_hiddens[i])])
        ###
        ...
        for idx in range(self.nsplits):
            ...
            # For those targets in the head (idx == 0) we only need to return their loss
            if idx == 0:
                softmaxed_head_res = softmaxed_all_head_res[running_offset:running_offset + len(
                    split_hiddens[idx])]
                entropy = -torch.gather(softmaxed_head_res,
                                        dim=1, index=split_targets[idx].view(-1, 1))
            # If the target is in one of the splits, the probability is the p(tombstone) * p(word within tombstone)
            else:
                softmaxed_head_res = softmaxed_all_head_res[running_offset:running_offset + len(
                    split_hiddens[idx])]
                ...
                # Calculate the softmax for the words in the tombstone
                tail_res = self.logprob(weight, bias, split_hiddens[idx], splits=[
                                        idx], softmaxed_head_res=softmaxed_head_res)
            ...
            total_loss = entropy.float().sum() if total_loss is None else total_loss + \
                entropy.float().sum()
